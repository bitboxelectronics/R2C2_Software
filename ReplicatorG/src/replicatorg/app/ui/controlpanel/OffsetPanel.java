...
public class OffsetPanel extends JPanel implements ActionListener, MouseListener {
	...
	protected EnumMap<AxisId, JTextField> positionFields = new EnumMap<AxisId, JTextField>(
			AxisId.class);
	...
	public class JogButton extends JButton {
		...
		public JogButton(String root, String tooltip) {
			...
			BufferedImage overImg = Base.getImage(
					"images/" + root + "Over.png", this);
			if (overImg != null) {
				setRolloverIcon(new ImageIcon(overImg));
				setRolloverEnabled(true);
			}
			BufferedImage downImg = Base.getImage(
					"images/" + root + "Down.png", this);
			if (downImg == null) {
				downImg = overImg;
			}
			if (downImg != null) {
			...
			Dimension imgSize = new Dimension(img.getWidth(null),
					img.getHeight(null));
			setSize(imgSize);
	...
	/**
	 * Create a jog-style button with the given name and tooltip. By default,
	 * the action name is the same as the text of the button. The button will
	 * emit an action event to the jog panel when it is clicked.
	 * 
	 * @param text
	 *            the text to display on the button.
	 * @param tooltip
	 *            the text to display when the mouse hovers over the button.
	 ...
	 * name is specified by the caller. The button will emit an action event to
	 * the jog panel when it is clicked.
	 * 
	 * @param text
	 *            the text to display on the button.
	 * @param tooltip
	...
	private JPanel buildPositionPanel() {
		...
		// R2C2: changes from "Center" to Home"s
		...
		for (final AxisId axis : machine.getModel().getAvailableAxes()) {
			...
			JButton homeButton = new JButton("Home " + axis.name());
			homeButton.setToolTipText("Home " + axis.name());
			homeButton.setActionCommand("Home " + axis.name());
			homeButton.addActionListener(this);
			positionPanel.add(homeButton);
		}

		// R2C2:
		// JButton zeroButton = new JButton("Make current position zero");
		JButton homeXYZButton = new JButton("Home XYZ");
		// zeroButton.setToolTipText("Mark Current Position as zero (0,0,0).  Will not move the toolhead.");
		// zeroButton.setActionCommand("Zero");
		homeXYZButton.setActionCommand("HomeXYZ");
		homeXYZButton.addActionListener(this);
		positionPanel.add(homeXYZButton, "growx");
	...
	private class FeedrateControl implements ActionListener, FocusListener,
			ChangeListener {
		...
		private FeedrateControl(String display, AxisId axis, JPanel parent) {
			...
			int maxFeedrate = (int) machine.getModel().getMaximumFeedrates()
					.axis(axis);
			int currentFeedrate = Math.min(maxFeedrate,
					Base.preferences.getInt(getPrefName(), 480));
	...
	public OffsetPanel(MachineInterface machine) {
		...
		JPanel xyPanel = new JPanel(new MigLayout("ins 0", "[]2[]2[]",
				"[]2[]2[]"));


			JButton zPlusButton = createJogButton("jog/Z+",
					"Jog Z axis in positive direction", "Z+");
			JButton zMinusButton = createJogButton("jog/Z-",
					"Jog Z axis in negative direction", "Z-");
		...
		setBorder(BorderFactory.createTitledBorder("Z Offset Calibration"));
	...
	synchronized public void updateStatus() {
		Point5d current = machine.getDriverQueryInterface().getCurrentPosition(
				false);
	...
	Pattern homeActionParser = Pattern.compile("Home ([XYZAB])");
	...
	public void actionPerformed(ActionEvent e) {
		...
		// Matcher centerMatch = homeActionParser.matcher(s);
		Matcher homeMatch = homeActionParser.matcher(s);
		if (jogMatch.matches()) {
			// If continuous jog mode, let the mouse listener interface handle
			// it.
			if (!continuousJogMode) {
				AxisId axis = AxisId.valueOf(jogMatch.group(1));
				boolean positive = jogMatch.group(2).equals("+");

				// R2C2: Changes to prevent the jogger from moving to negative
				// positions and farther the the build volume allows
				// current.setAxis(axis, current.axis(axis) +
				// (positive?jogRate:-jogRate));
				
				if (true){
					double safety = Math.max(Math.min(
						(double) ((current.axis(axis) + (positive ? jogRate
								: -jogRate))), (double) (machine.getModel()
								.getBuildVolume().getAxis(axis))), 0.0);

					current.setAxis(axis, safety);

				}
				else{current.setAxis(axis, current.axis(axis) + (positive?jogRate:-jogRate));}
				
				// Base.logger.fine("New position: "+current.toString());
				double f = feedrate.axis(axis);
				// Exception: XY feedrate is assumed to be X feedrate
				// (symmetrical)
				if (axis.equals(AxisId.Y)) {
					f = feedrate.axis(AxisId.X);
				}
				machine.runCommand(new replicatorg.drivers.commands.SetFeedrate(
						f));
				machine.runCommand(new replicatorg.drivers.commands.QueuePoint(
						current));
			}
			...
			// machine.runCommand(new
			// replicatorg.drivers.commands.SetFeedrate(f));
			// machine.runCommand(new
			// replicatorg.drivers.commands.QueuePoint(current));

			EnumSet<AxisId> axes = EnumSet.noneOf(AxisId.class);
			axes.add(axis);
			machine.runCommand(new replicatorg.drivers.commands.HomeAxes(axes,
					LinearDirection.POSITIVE));
			...
			machine.runCommand(new replicatorg.drivers.commands.SetCurrentPosition(
					new Point5d()));
		} else if (s.equals("HomeXYZ")) {
	...
	public void mousePressed(MouseEvent e) {
		if (continuousJogMode) {
			Point5d current = machine.getDriverQueryInterface()
